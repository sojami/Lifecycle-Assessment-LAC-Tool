...
KNOWN_MAKES = [
    ...
    "Chrysler", "Dodge", "Ferrari", "Fiat", "Ford", "Genesis", "GMC", "Honda", "Hyundai",
    "Infiniti", "Jaguar", "Jeep", "Kia", "Lamborghini", "Land Rover", "Lexus", "Lincoln",
    "Lucid", "Maserati", "Mazda", "Mercedes-Benz", "Mini", "Mitsubishi", "Nissan", "Polestar",
...
MAKE_ALIASES = {
    ...
    "lambo": "Lamborghini",
    "lambogini": "Lamborghini",
    "lamborgini": "Lamborghini",
    "lambogin": "Lamborghini",
    "merc": "Mercedes-Benz",
...
def get_known_makes() -> list[str]:
    return sorted(set(KNOWN_MAKES) | {vehicle["make"] for vehicle in KNOWN_VEHICLE_CATALOG[1:]})


def normalize_make_input(make: str) -> str:
    normalized_make = (make or "").strip().lower()
    if not normalized_make:
        return ""
    if normalized_make in MAKE_ALIASES:
        return MAKE_ALIASES[normalized_make]

    known_makes = get_known_makes()
    lowercase_map = {known_make.lower(): known_make for known_make in known_makes}
    if normalized_make in lowercase_map:
        return lowercase_map[normalized_make]

    close_match = get_close_matches(normalized_make, list(lowercase_map.keys()), n=1, cutoff=0.72)
    if close_match:
        return lowercase_map[close_match[0]]
    return ""


def is_valid_make(make: str) -> bool:
    return bool(normalize_make_input(make))
...
def find_make_class_match(make: str, vehicle_class: str) -> dict | None:
    ...
    for vehicle in KNOWN_VEHICLE_CATALOG[1:]:
        if vehicle["make"] == canonical_make and vehicle["vehicle_class"] == normalized_class:
            return vehicle
    return None
